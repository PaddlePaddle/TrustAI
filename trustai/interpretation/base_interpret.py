...
class Interpreter(abc.ABC):
    ...
    def __init__(self, paddle_model: callable, device: str = 'gpu', **kwargs):
        self.device = device
        self.paddle_model = paddle_model
        self.predict_fn = None

        assert isinstance(self.device, str) and self.device in [
            'cpu', 'gpu'
        ], "The format of the ``devices`` should be like ``cpu``, ``gpu`` etc."
