...
parser.add_argument("--warmup_steps", default=1000, type=int, help="Linear warmup steps over the training process.")
...
def read(data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if len(splits) == 3:
                text_a, text_b, label, weight = splits + [1]
            else:
                text_a, text_b, label, weight = splits

            yield {"text_a": text_a, "text_b": text_b, "label": int(label), 'weight': float(weight)}
...
def train():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            input_ids, token_type_ids, labels, weights = batch['input_ids'], batch['token_type_ids'], batch[
