...
use_f_times = -1
use_l_times = -1
#index_dict = {"error_label": -11, "correct_times": -10, "correct_ratio": -9, "avg_probs": -8, "label_var": -7, "max_label_probs": -6, 
#                "min_label_probs": -5, "forgetting_times": -4, "learnt_times": -3, "first_forget": -2, "first_learn": -1}

#feature_list = ["correct_times", "correct_ratio", "avg_probs", "label_var", "max_label_probs", "min_label_probs", 
#                    "forgetting_times", "learnt_times", "first_forget"]

input_path = "./outputs/"
input_file = "output_data.json.result"
attr1 = "avg_probs"
attr2 = "label_var" # [label_var, low_bias]
...
def build_dict(header_list, init_dict={}, standard_dict={}, error_dict={}):
    ...
    specical_dict_f = {"forgetting_times-+": [[] for i in range(2)], "forgetting_times-": [[] for i in range(2)], "forgetting_times0": [[] for i in range(2)], 
                    "forgetting_times1": [[] for i in range(2)], "forgetting_times2": [[] for i in range(2)], "forgetting_times3": [[] for i in range(2)]}
...
special_marker_dict = {"forgetting_times-+": ["black", "X", 3], "forgetting_times-": ["c", "v", 3], "forgetting_times0": ["b", ">", 3],  
                        "forgetting_times1": ["r", "<", 3], "forgetting_times2": ["g", "D", 3], "forgetting_times3": ["orange", "^", 3], 
...
def fill_in_specical_dict_f(specical_dict_f, cri_cnt, feature1, feature2, forgetting_times, correct_ratio):
    if forgetting_times == 0:
        if correct_ratio == 0:
            specical_dict_f["forgetting_times-"][0].append([feature1])
            specical_dict_f["forgetting_times-"][1].append([feature2])
            cri_cnt[1] += 1
            ...
            specical_dict_f["forgetting_times0"][0].append([feature1])
            specical_dict_f["forgetting_times0"][1].append([feature2])
            cri_cnt[3] += 1
    elif forgetting_times == 1:
        specical_dict_f["forgetting_times1"][0].append([feature1])
        specical_dict_f["forgetting_times1"][1].append([feature2])
        cri_cnt[4] += 1
    elif forgetting_times == 2:
        specical_dict_f["forgetting_times2"][0].append([feature1])
        specical_dict_f["forgetting_times2"][1].append([feature2])
        cri_cnt[5] += 1
    else:
        specical_dict_f["forgetting_times3"][0].append([feature1])
        specical_dict_f["forgetting_times3"][1].append([feature2])
        cri_cnt[6] += 1
...
def run_plot(in_path, in_file, dim1, dim2, criterion=False, threshold=0, dirty_label=False, use_f_times=-1, use_l_times=-1):
    ...
    if use_f_times != -1 and use_l_times != -1:
        print("Only one of forgetting_times and learnt_times can be used!")
        return
    if use_f_times != -1:
        forgetting_times_threshold = use_f_times # related to use_f_times 3
    if use_l_times != -1:
    ...
    for idx, line in enumerate(_input):
        ...
        forgetting_times = int(line[index_dict["forgetting_times"]])
        ...
        if use_f_times != -1:            
            if forgetting_times >= forgetting_times_threshold:
                time_cnt[0] += 1
                specical_dict_f, time_cnt = fill_in_specical_dict_f(specical_dict_f, time_cnt, feature1, feature2, forgetting_times, correct_ratio)
                
        elif use_l_times != -1:
            if learnt_times >= learnt_times_threshold:
                time_cnt[0] += 1
                specical_dict_l, time_cnt = fill_in_specical_dict_l(specical_dict_l, time_cnt, feature1, feature2, learnt_times, correct_ratio)
    ...
    if use_f_times != -1:
        print("sample number under forgetting_times is: ", time_cnt)
    elif use_l_times != -1:
    ...
    if criterion:
        ...
        plt.title(dim1 + " - " + dim2 + " plot forgetting_times greater than " + str(forgetting_times_threshold))
    elif use_l_times != -1:
        plt.title(dim1 + " - " + dim2 + " plot learnt_times greater than " + str(learnt_times_threshold))
    else:
        plt.title(dim1 + " - " + dim2 + " plot")
    if use_f_times != -1:
        plt.savefig('./outputs/ftimes' + str(forgetting_times_threshold) + '.' + out_file + '.png')
    elif use_l_times != -1:
