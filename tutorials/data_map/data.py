...
def convert_pointwise_example(example,
    ...
    sep_ids = len(input_ids) - 1

    #print("input_ids:",input_ids) 
    if not is_test:
        label = np.array([example[l_name]], dtype="int64")
        s_label = np.array([example[s_name]],dtype="int64")
        sep_ids = np.array([sep_ids], dtype="int64")
        #print('label', label)
        return input_ids, token_type_ids, label, s_label, sep_ids
    else:
